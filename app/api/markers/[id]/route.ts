...
export async function GET(
  ...
  try {
    ...
    const result = await query<Marker>(
      'SELECT * FROM markers WHERE marker_id = $1',
      [id]
    )

    if (!result.rowCount || result.rowCount === 0) {
      return NextResponse.json(
        { success: false, error: '마커를 찾을 수 없습니다.' },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      { success: false, error: '마커 조회 중 오류가 발생했습니다.' },
      { status: 500 }
    )
  }
}
...
export async function PUT(
  ...
  try {
    ...
    values.push(id)
    const result = await query<Marker>(
      `UPDATE markers SET ${fields.join(', ')} WHERE marker_id = $${idx} RETURNING *`,
      values
    )
    ...
    return NextResponse.json(
      { success: false, error: '마커 수정 중 오류가 발생했습니다.' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await query(
      'DELETE FROM markers WHERE marker_id = $1 RETURNING marker_id',
      [id]
    )
    ...
    return NextResponse.json({ success: true, message: '마커가 삭제되었습니다.' })
  } catch (error) {
    console.error('마커 삭제 오류:', error)
    return NextResponse.json(
      { success: false, error: '마커 삭제 중 오류가 발생했습니다.' },
      { status: 500 }
    )
  }
}
